...
- 遵守客观物理规律，该怎么决定就怎么决定。主导因子：距离球门、防守压力、角度、球高
- 球员属性也自然影响决策——自信高就更敢冒险、团队高就更听战术安排。但属性不是决策主驱动，物理条件才是
- 研究先于编码——搜学术文献（Google Scholar、xG 模型、xT/VAEP 研究）找依据，不拍脑袋
- "角色"不是固定标签——同一球员在不同战术部署下行为不同。将来会有的"职能标签"是战术系统的一部分

### 严禁事项
- **禁止任何强制参数**：所有决策概率必须通过物理公式自然推导。`return 0`、`return 1`、`if (x < MAGIC_NUMBER) 截断`、硬编码`MIN_XXX`阈值、拍脑袋的`if (v === 'XXX') ...`——这些全部禁止。如果某个场景的概率分布不合理，修改效用函数的连续数学形式，不允许用离散判定截断
- **禁止建议"强行限制"作为方案**：除非用户主动提出，永远不要把加阈值、截断、设零等强制手段作为修复选项
...
```
        物理条件        战术指令(将来)     球员属性(决策+执行)
        ────────        ────────────       ──────────────────
        
距离→xG   ──→ 射门效用  shootWillingness   自信高→更敢射
zone     ──→ 传球效用                     团队高→更听战术
pressure ──→ 盘带效用                     执行层: shoot.js pass.js 等
defDist  ──→ 抢断检查                     
coords   ──→ 争顶检查(将来)               
...
射门/传球/盘带三行独立计算效用值，不互加减，用 softmax 比较。属性主要管执行质量，但也自然影响决策偏好。
